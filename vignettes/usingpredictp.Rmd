...
```{r detfunvalues}
detfun = plotdetfn(vismodel,eg_viscov[1,],addCI=FALSE)
detfun
```
...
time `plotavail` is called. Also, note that when you assign the call to the function `plotavail` 
to some variable (e.g., to `CDF`, or `CDF0`, or `PDF`, or `PDF0` above) you get a data frame with 
the coordinates used to make the plot. For example: 
```{r FO}
CDF0
```
